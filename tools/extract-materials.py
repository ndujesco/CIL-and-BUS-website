...
def add(doc_id, course, ref, title, kicker, parts, also=()):
    ...
    for part in parts:
        ...
        html = _number_blocks(html, doc_id, n)
        n += len(toc)
        made.append({
...
def _number_blocks(html, doc_id, start):
    """And every paragraph, list item and equation, so a citation can point at
    the line rather than at the section it sits in."""
    seen = [0]

    def mark(m):
        seen[0] += 1
        tag = m.group(1)
        at = ' id="' + doc_id + "-p" + str(start) + "-" + str(seen[0]) + '"'
        return ("<" + tag + at + ">") if tag else ('<div class="eqn"' + at + ">")

    return BODY.sub(mark, html)
